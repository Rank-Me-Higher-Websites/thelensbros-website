...
// Slim, dark glass-morphism nav with blue/gold accent on scroll
  ...
  return (
    <nav
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
        scrolled
          ? 'bg-[oklch(0.08_0.015_270/0.92)] backdrop-blur-xl border-b border-white/5 shadow-lg shadow-black/20'
          : 'bg-transparent'
      }`}
    >
      {/* Slimmer container: h-12 mobile, h-14 desktop */}
      <div className="container flex items-center justify-between h-12 md:h-14">
        ...
        <div className="hidden lg:flex items-center gap-0.5">
          {navLinks.map((link) =>
            link.children ? (
              ...
              >
                ...
                >
                  <div className="bg-[oklch(0.1_0.015_270/0.95)] backdrop-blur-xl border border-white/10 rounded-lg py-1.5 min-w-[210px] shadow-2xl">
          ...
          <Link
            href="/contact"
            className="ml-3 px-4 py-1.5 text-[13px] font-semibold font-display bg-gradient-to-r from-[oklch(0.65_0.2_250)] to-[oklch(0.75_0.14_80)] text-white rounded-md hover:opacity-90 transition-opacity"
          >
            Book Now
          </Link>
        </div>
      ...
      >
        <div className="bg-[oklch(0.08_0.015_270/0.98)] backdrop-blur-xl border-t border-white/5 px-4 py-3 space-y-0.5">
          {navLinks.map((link) =>
            link.children ? (
              <div key={link.label}>
                <button
                  onClick={() => setMobileServicesOpen(!mobileServicesOpen)}
                  className="flex items-center justify-between w-full px-3 py-2.5 text-sm font-medium text-white/70 hover:text-white transition-colors"
                >
                  {link.label}
                  ...
                  <div className="pl-4 space-y-0.5 pb-1">
                    {link.children.map((child) => (
                      <Link
                        key={child.href}
                        href={child.href}
                        className="block px-3 py-2 text-sm text-white/50 hover:text-white transition-colors"
                      >
                        {child.label}
                      </Link>
                    ))}
          ...
          <Link
            href="/contact"
            className="block mx-3 mt-2 px-5 py-2.5 text-center text-sm font-semibold font-display bg-gradient-to-r from-[oklch(0.65_0.2_250)] to-[oklch(0.75_0.14_80)] text-white rounded-md"
          >
            Book Now
          </Link>
        </div>
